...
def load_csv(filepath):
    """Parse the custom 3-line-header CSV format used by this lab."""
...
class c45:
    def __init__(self, metric='gain_ratio', threshold=0.0):
        """
        metric    : 'gain' for Information Gain, 'gain_ratio' for Information Gain Ratio
        threshold : stop splitting when best score <= threshold
        """
        self.metric = metric
    ...
    # ── numeric best-threshold search (O(n log n) via incremental scan) ──────────
    ...
    def _score(self, data, attr, class_col):
        """Return (score, threshold_or_None) for one attribute."""
        n = len(data)
    ...
    def _build(self, data, attributes, class_col, default):
        ...
        if is_numeric:
            # Numeric: keep attribute available for deeper splits with new threshold
